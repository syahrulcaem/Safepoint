class Dashboard {
  ...
  load(filter) {
    ...
    this.content1_ =
      `<div class="row">
			<div class="col-12 col-lg-3 search-wrap">` +
      this.searchBox +
      `</div>

			<div class="col-12 col-lg-3">
				<div class="row" style="overflow:auto">
				<div class="col"><select class="form-control" style="width: 100%;height:48px;" id="filter_id_bptd"  placeholder="d"></select></div>
				</div>
			</div>
  ...
  setBusTooltip(busMarker, item) {
    ...
    var infoVehicle =
      ...
      item.direction +
      "</td></tr>" +
      "<tr><td>Nama Driver</td><td>" +
      (item.driver_nm === "undefined" ? item.driver_nm : "-") +
      "</td></tr>" +
      "<tr><td>Speed</td><td>" +
      item.speed +
      " km/h</td></tr>" +
      '<tr><td>Battery</td><td><div style="border:1px solid black"><div style="background-color:' +
      batt_color +
      ";width:" +
      item.battery_percent.toFixed(0) +
      '%">' +
      item.battery_percent.toFixed(2) +
      "</div></div></td></tr>" +
      "<tr><td>Last Updated</td><td>" +
      $.timeago(item.gps_time) +
      " <br/>" +
      item.gps_time +
      "</td></tr>" +
      "</table>";
  ...
  setBusDataSource(datasource) {
    ...
    $.each(datasource, function (index, obj) {
      if (thisClass.objtype[thisClass.objtype.length - 1] != obj.group_nm) {
        ...
        ng = 1;
        thisClass.vdata_bus.results[
          thisClass.vdata_bus.results.length - 1
        ].children = [
          {
            id: obj.gps_sn,
            text:
              thisClass.html_logobus +
              " " +
              obj.nopol +
    ...
    $.each(thisClass.vdata_bus.results, function (index, item) {
      thisClass.vdata_bus.results[index].text =
        item.text + " (" + item.children.length + " armada)";
    });
  }
  ...
  loadDataPetugas() {
    var thisClass = this;
    if (typeof thisClass.baseUrl == "undefined")
      thisClass.baseUrl = "https://mitradarat-fms.dephub.go.id";
    $.ajax({
      type: "POST",
  ...
  fillPetugas(ret) {
    ...
    ret
      .filter(function (el) {
        return (
          el.name.toLowerCase().includes($("#cariuser").val().toLowerCase()) ||
          el.lokasi.toLowerCase().includes($("#cariuser").val().toLowerCase())
        );
      })
        ...
        element +=
          `<li class="active">
			  <a href="#">
				  <div class="d-flex align-items-start">
					  <div class="flex-shrink-0 user-img ` +
          (item.status == "offline" ? "" : "online") +
          ` align-self-center me-3">
						  <img src="` +
          item.user_mobile_photo +
          ...
          `</h5>
					  ...
					  <div class="flex-shrink-0">
						  <div class="call-this-user" data-id="` +
          item.id +
          `"><i class="bx bx-phone-call"></i></div>
					  </div>
				  </div>
			  </a>
		  </li>`;
  ...
  activateCall(completion = null) {
    ...
    thisClass.api = new JitsiMeetExternalAPI(
      thisClass.domain,
      thisClass.options
    );
    ...
    thisClass.api.addEventListener("videoConferenceJoined", function (e) {
      thisClass.api.executeCommand(
        "avatarUrl",
        thisClass.options.userInfo.avatarURL
      );
  ...
  makeGroupCall() {
    ...
    $.ajax({
      ...
      data: {
        [thisClass.csrfName]: thisClass.csrfHash,
        sender: thisClass.displayName,
        room: thisClass.roomName,
        handler: "",
      },
