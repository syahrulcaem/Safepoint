class Dashboard {
    ...
    load(filter) {
        var thisClass = this;
        // console.log('Dashboard FMS load');
        ...
        this.searchBox = `<span class="dropdown" id="menu-main">
                            ...
                            <img src="/assets/img/LogoOnly.png" style="width: 44px; height: 44px; margin-top: 2px;">
                            </span>
                        </span>`;
        ...
        */
        ...
        /* 
        edited by rey 2024-08-08
        - fixing ui for role pop and bts
        */
       this.contentFMS_ = `<div class="row">
                                <div class="rounded-circle shadow p-0" style="width: 44px; background: white; height: 44px; margin-top: 2px;">`+ this.searchBox + `</div>
                                <div class="col-6 col-lg-3">
                                    <div class="row" style="overflow:auto">
                                        <div class="col"><select class="form-control" style="width: 100%;height:48px;" id="filter_bptd"  placeholder="d"></select></div>
                                    </div>
                                </div>
                            </div>`;
       this.contentBTS_ = `<div class="row">
                                <div class="rounded-circle shadow p-0" style="width: 44px; background: white; height: 44px; margin-top: 2px;">`+ this.searchBox + `</div>
                                <div class="col-6 col-lg-3">
                                    <div class="row" style="overflow:auto">
    ...
    setBusTooltip(busMarker, item) {
        ...
        var addHtml2 = '';
        var addHtml3 = '';
        // var nospda = 'belum terinput';
        var vspda_status = '';

        if(typeof item.spda_status!='undefined'){
            switch(item.spda_status){
                case '0': vspda_status = 'Dalam perjalanan'; break;
                case '1': vspda_status = 'Selesai'; break;
                default: vspda_status = 'Belum terinput'; break;
            }
            addHtml3 = `<tr><td class="fw-bold">SPDA Status</td><td>: ${vspda_status}</td></tr>
                        <tr><td class="fw-bold">Driver</td><td>: ${item.driver_name}</td></tr>
                        <tr><td class="fw-bold">Penumpang</td><td>: ${item.passenger}</td></tr>
                        <tr><td class="fw-bold">No. Uji</td><td>: ${item.no_uji}</td></tr>
                        <tr><td class="fw-bold">Tgl Exp Uji</td><td>: <span class="d-inline-block align-top">${item.tgl_exp_uji}<br><span class="text-muted">(${item.cap_exp_uji})</span></span></td></tr>
                        <tr><td class="fw-bold">No. KPS</td><td>: ${item.no_kps ?? '-'}</td></tr>
                        <tr><td class="fw-bold">Tgl Exp KPS</td><td>: <span class="d-inline-block align-top">${item.tgl_exp_kps  ?? '-'}<br><span class="text-muted">(${item.cap_exp_kps  ?? '-'})</span></span></td></tr>`;
        }
        
        if(typeof item.operator!='undefined'){
            operator = item.operator;
            addHtml = `<tr><td colspan="2" class="text-center">${operator}</td></tr>`;
        }

        if(typeof item.toward!='undefined'){
            addHtml2 = '<tr><td>Position</td><td class="bus-pos"><span class="badge bg-soft-success text-success">' + arrBusStop[item.old_shel_t] + '</span>'+
        ...
        if(typeof item.bus_code!='undefined'){
            bus_code = item.bus_code;
        }
        if(typeof item.name!='undefined'){
            bus_code = item.name;
        }
        if(typeof item.bus_route!='undefined'){
            bus_route = item.bus_route;
        }
        if(typeof item.kor!='undefined'){
            bus_route = item.kor;
        }

        // console.log(item);
        var infoVehicle = `<div class="table-responsive">
                                <table class="bus-tooltip">${addHtml}
                                <colgroup>
                                    <col style="width: 30%;">
                                    <col style="width: 70%;">
                                </colgroup>
                                    <tr><td class="fw-bold">Jenis</td><td>: ${item.jenroute}</td></tr>
                                    <tr><td class="fw-bold">IMEI GPS</td><td>: ${item.gps_sn}</td></tr>
                                    <tr><td class="fw-bold">Jalur</td><td>: <span class="d-inline-block text-truncate text-wrap align-top" style="max-width: 10em;">${bus_route}</span></td></tr>
                                    <tr><td class="fw-bold">Bus</td><td>: ${bus_code}</td></tr>
                                    <tr><td class="fw-bold">No. Kendaraan</td><td>: <span class="badge bg-dark text-white fs-6">${item.nopol.replace(/([a-zA-Z]+)(\d+)([a-zA-Z]+)/, '$1 $2 $3')}</span></td></tr>
                                    ${addHtml3}
                                    <tr><td class="fw-bold">Angle</td><td>: ${item.direction}&deg;</td></tr>
                                    <tr><td class="fw-bold">Speed</td><td>: ${item.speed} km/h</td></tr>
                                    ${addHtml2}
                                    <tr><td class="fw-bold">Last Updated</td><td>: <span class="d-inline-block align-top">${item.gps_time}<br><span class="text-muted">(${moment(item.gps_time).fromNow()})</span></span></td></tr>
                                </table>
                            </div>`;
    ...
    activateCall(completion = null) {
        ...
        thisClass.options.roomName = thisClass.defaultRoomName + thisClass.makeid(8);
