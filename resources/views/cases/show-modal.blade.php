                ...
                @if ($case->status === 'NEW')
                    ...
                    <div class="mb-3">
                        <form method="POST" action="{{ route('cases.dispatch-multi', $case) }}"
                            id="dispatch-form-modal">
                            @csrf
                            <div class="mb-3">
                                <label class="form-label">Pilih Unit (Multi-Select)</label>
                                <div class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
                                    @foreach ($units as $unit)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="checkbox" name="unit_ids[]"
                                                value="{{ $unit->id }}" id="unit_{{ $unit->id }}_modal">
                                            <label class="form-check-label" for="unit_{{ $unit->id }}_modal">
                                                {{ $unit->name }} <span
                                                    class="text-muted">({{ $unit->type }})</span>
                                            </label>
                                        </div>
                                    @endforeach
                                </div>
                                <small class="text-muted">
                                    Pimpinan unit akan menugaskan petugas
                                </small>
                            </div>
                            <div class="mb-3">
                                <label for="notes" class="form-label">Catatan untuk Pimpinan</label>
                                <textarea name="notes" id="notes" class="form-control" rows="3"
                                    placeholder="Catatan untuk pimpinan unit..."></textarea>
                            </div>
                            <button type="submit" class="btn btn-primary w-100">
                                <i class="bx bx-send me-1"></i> Kirim ke Unit Terpilih
                            </button>
                        </form>
                    </div>
                @endif
            ...
            <div class="card-body">
                ...
                @if ($case->lat && $case->lon)
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <span class="fw-medium">{{ number_format($case->lat, 6) }},
                            {{ number_format($case->lon, 6) }}</span>
                    </div>
                @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const dispatchFormModal = document.getElementById('dispatch-form-modal');
        if (dispatchFormModal) {
            dispatchFormModal.addEventListener('submit', function(e) {
                const checkboxes = dispatchFormModal.querySelectorAll(
                    'input[name="unit_ids[]"]:checked');
